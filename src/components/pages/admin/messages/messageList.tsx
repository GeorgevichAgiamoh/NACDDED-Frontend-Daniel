...
import { adminUserEle, msgStat, msgThread, dioceseBasicinfo, dioceseGeneralinfo, dioceseSecretaryInfo, } from "../../../classes/models"
...
function PickMsgReceiver(prop:{closy:()=>void, rdy:(name:string,id:string,email:string,subject:string)=>void} ) {
    ...
    const[dioceses,setDioceses] = useState<dioceseBasicinfo[]>([])
    ...
    return <div style={{
        ...
        <div className="hlc" style={{
           ...
           <div style={{
             ...
             <EditTextFilled hint={`Search diocese by name`} value={search} min={5} recv={(v)=>{
                    setSearch(v)
                }} />
           </div>
            <Mgin right={10}/>
            <Btn txt="Search" width={100} onClick={()=>{
                ...
                makeRequest.get('searchMember',{
                    ...
                    if(task.isSuccessful()){
                        ...
                        for(const key in task.getData()){
                            const dbi = new dioceseBasicinfo(task.getData()[key]['b'])
                            dbi.setGeneralData(new dioceseGeneralinfo(task.getData()[key]['g']))
                            dbi.setSecretaries([new dioceseSecretaryInfo(task.getData()[key]['s'])])
                            tem.push(dbi)
                        }
                        setDioceses(tem)
                    }else{
        ...
        <Mgin top={20}/>
        <mye.Tv text={`Choose Diocese`} size={14} />
        <Mgin top={10}/>
        <div style={{
            width:'100%',
            flex:1,
        }}>
            {
                ...
                dioceses.map((ele,i)=>{
                    return <div id="clk" key={myKey+i+0.121} onClick={()=>{
                        prop.rdy(ele.getName(),ele.getDioceseID(),ele.secretaries![0].getEmail(),subject)
                    }} style={{
                        width:'100%',
                        marginBottom:10,
                        ...
                        <LrText 
                        left={<mye.BTv text={ele.getName()} size={14} />}
